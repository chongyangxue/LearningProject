...
public class LongConnection {
	 ...
	 * 这个问题的一个解决方法就是在每次请求之前检查该连接是否已经存在了过长时间，可能已过期。 但是这个方法会使得每次请求都增加额外的开销。HTTP
	 * Client4.0的ThreadSafeClientConnManager 提供了
	...
	private static void test12() throws IOException, KeyManagementException,
		...
		PlainConnectionSocketFactory sf = PlainConnectionSocketFactory
				.getSocketFactory();// Plain：简洁的
		Socket socket = sf.createSocket(clientContext);
		HttpHost target = new HttpHost("localhost");
		InetSocketAddress remoteAddress = new InetSocketAddress(
				InetAddress.getByAddress(new byte[] { 127, 0, 0, 1 }), 80);
		sf.connectSocket(1000, socket, target, remoteAddress, null,
				clientContext);
	 ...
	 * HttpClient代理服务器配置
	 * 
	 * 使用代理服务器最简单的方式就是，指定一个默认的proxy参数。 我们也可以让HttpClient去使用jre的代理服务器。
	...
	private static void test13() {
		...
		CloseableHttpClient httpclient2 = HttpClients.custom()
				.setRoutePlanner(routePlanner2).build();
		...
		CloseableHttpClient httpclient3 = HttpClients.custom()
				.setRoutePlanner(routePlanner3).build();
